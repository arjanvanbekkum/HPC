...
EXEC dbo.sp_add_job  
   @job_name = N'BackupHPCMonitoringDatabase',   
   @enabled = 1,   
...
SELECT @name = ''HPCMonitoring'' + ''_'' + @year + @month + @day + @hour + @minute + @second

SET @pathwithname = @path + @namE + ''.bak''

BACKUP DATABASE [HPCMonitoring] TO DISK = @pathwithname WITH FORMAT, MEDIANAME = ''SQLServerBackups'', NAME = ''Full Backup of HPCMonitoring''; 
GO  
',   
    @retry_attempts = 5,   
    @retry_interval = 5 ;   
...
EXEC dbo.sp_add_schedule  
    @schedule_name = N'DailyFullBackup_HPCMonitoring',   
    @freq_type = 4,   
    @freq_interval = 1,   
