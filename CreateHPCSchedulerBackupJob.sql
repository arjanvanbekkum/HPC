...
EXEC dbo.sp_add_job  
   @job_name = N'BackupHPCSchedulerDatabase',   
   @enabled = 1,   
...
SELECT @name = ''HPCScheduler'' + ''_'' + @year + @month + @day + @hour + @minute + @second

SET @pathwithname = @path + @namE + ''.bak''

BACKUP DATABASE [HPCScheduler] TO DISK = @pathwithname WITH FORMAT, MEDIANAME = ''SQLServerBackups'', NAME = ''Full Backup of HPCScheduler''; 
GO  
',   
    @retry_attempts = 5,   
    @retry_interval = 5 ;   
...
EXEC dbo.sp_add_schedule  
    @schedule_name = N'DailyFullBackup_HPCScheduler',   
    @freq_type = 4,   
    @freq_interval = 1,   
