...
EXEC dbo.sp_add_job  
   @job_name = N'BackupHPCReportingDatabase',   
   @enabled = 1,   
...
SELECT @name = ''HPCReporting'' + ''_'' + @year + @month + @day + @hour + @minute + @second

SET @pathwithname = @path + @namE + ''.bak''

BACKUP DATABASE [HPCReporting] TO DISK = @pathwithname WITH FORMAT, MEDIANAME = ''SQLServerBackups'', NAME = ''Full Backup of HPCReporting''; 
GO  
',   
    @retry_attempts = 5,   
    @retry_interval = 5 ;   
...
EXEC dbo.sp_add_schedule  
    @schedule_name = N'DailyFullBackup_HPCReporting',   
    @freq_type = 4,   
    @freq_interval = 1,   
